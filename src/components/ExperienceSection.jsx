...
const workExperience = [
  {
    ...
    description: 'Worked on API development, database integration, and backend features using Laravel.',
    logo: multisysLogo,
  },
  {
    ...
    description: 'Assisted in preparing AutoCAD materials for the ASEAN WorldSkills 2025 competition, helped with technical documentation and product research, and supported software testing with the engineering team.',
    logo: hytecLogo,
  },
];
...
export default function ExperienceSection() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (timelineContainerRef.current) {
        timelineContainerRef.current.removeEventListener('scroll', scrollHandler);
      }
    };
  }, [activeTab]);
    ...
    data.map((item, idx) => (
      <li key={idx} className={`relative flex items-start${idx !== data.length - 1 ? ' mb-12' : ''}`}>
        ...
        <div className="ml-0 pr-8 -mt-1">
          ...
          {item.description && (
            <div className="text-white text-sm font-normal leading-relaxed">{item.description}</div>
          )}
        </div>
      </li>
    ));
    ...
    <section className="container max-w-3xl mx-auto px-4 md:px-0 pt-10">
      ...
      <div className="bg-transparent border border-white rounded-lg overflow-hidden shadow-sm">
        ...
        <div
          ...
          style={{ position: 'relative', background: 'transparent', marginRight: '0' }}
